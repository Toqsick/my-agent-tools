...
description: |
  Use when setting up event-driven agent runs via webhook subscriptions, wiring an external service (GitHub, CI, calendar) to trigger a Hermes run on events, or managing existing webhook endpoints.
  NOT for polling-based triggers, one-off scripts, or non-event-driven automation — use cron or terminal for those.
  Set up and manage webhook subscriptions for event-driven Hermes agent runs.
version: 1.1.0
platforms:
- linux
- macos
...
trigger_keywords: ['event', 'driven', 'webhook', 'agent', 'runs']
keywords: ['event', 'driven', 'webhook', 'agent', 'runs']
