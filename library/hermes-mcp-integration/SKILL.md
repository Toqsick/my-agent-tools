...
version: 1.0.0
description: |
  Use when you integrate Hermes V7's native MCP client with stdio or HTTP/SSE MCP servers — adding a server to config.yaml, debugging tool discovery, wiring `mcp__server__tool` calls, or troubleshooting tool-not-found errors at agent startup.
  NOT for writing a NEW custom MCP server (use mcp-server-authoring), LangChain tool integrations, or pre-V7 Hermes versions without the native MCP client.
  Hermes V7 native MCP client integration: add/configure MCP servers (stdio/HTTP), discover tools automatically, invoke them via `mcp__server__tool` syntax, debug discovery failures.
...
author: Hermes Agent
license: MIT
trigger_keywords: ['server', 'tool', 'hermes', 'native', 'client']
keywords: ['server', 'tool', 'hermes', 'native', 'client']
last_curated: '2026-07-23'
curated_by: 'Yuno'
related_skills: ['native-mcp', 'mcp-server-authoring', 'a2a-bridge']
---
