...
name: simplify-code
description: |
  Use when cleaning up a recent code change through parallel readability, duplication, and correctness reviews, then applying only independently justified improvements.
  NOT for broad rewrites, feature development, or accepting overlapping agent edits without reconciling them against tests and the original behavior.
  Runs a three-agent cleanup pattern that identifies simplifications, consolidates findings, and verifies the resulting code.
version: 1.0.0
...
trigger_keywords: ['code', 'agent', 'cleaning', 'recent', 'change']
keywords: ['code', 'agent', 'cleaning', 'recent', 'change']
