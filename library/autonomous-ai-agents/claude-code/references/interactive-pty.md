# Interactive PTY Mode (tmux Orchestration)

Interactive mode gives you a full conversational REPL with follow-up prompts, slash commands, and real-time observation. **Requires tmux orchestration** — `terminal(pty=true)` works but tmux gives you `capture-pane` for monitoring and `send-keys` for input.
