---
name: multi-agent-pitfalls-cheatsheet
...
triggers:
- Before spawning delegate_task (any kind: research, audit, build, code review)
- User asks for multi-agent or parallel agent or research team or 3-expert audit
- Post-subagent-run synthesis (verify claims)
- phantom fix detected
- subagent timeout or hung or returned no output
version: 1.1.4
author: Yuno (consolidated from .archive/multi-agent-research)
license: MIT
platforms:
- linux
- macos
...
metadata:
  hermes:
    ...
    - subagents
    changelog:
    - 1.1.4 (2026-07-03): Pitfall #30 enhanced with 70+min YUNO V3 audit timeout proof; new Pitfall #34 added — "Single-file code audit: parent-direct beats subagent" with concrete YUNO V3 evidence (30 min wall-clock vs 70+ min subagent hang).
    - 1.1.2 (2026-07-03): Markdown Compression Iterations pitfall added — Parent-Direct slim-down of content-dense skills takes ~10 patch iterations to land under target; target ~80% of size on first pass. Proven on `claude-design` 25.2KB→11.9KB. Third proven-impact row added for creative-class slim-down.
    - 1.1.1 (2026-07-03): Skill-Slim-Down Protocol Step 6 — added inline phantom-ref detection command (catches broken See-Also links pre-slim), frontmatter byte-identity diff, content-loss heading/table counters. Proven on `hermes-v7-sse-server` slim-down (22.9KB→12.2KB, 1 phantom ref caught and turned into a real reference).
    - 1.1.0 (2026-07-03): Round-2 patterns added — Re-Scan Fresh, Skill-Exclusion-List, Stub-vs-Block-Replace, Subject-Index Acceptance, Subagent Honesty, Smaller-Candidate Tolerance, 3% Target Overshoot. Cross-referenced with `skill-library-maintenance` v1.1.0.
    - 1.0.0 (2026-07-02): Initial release with consolidated 20 pitfalls + 4 new patterns from live demo (Hybrid Pre-Scan, Controlled Write, 3-Tier Verification, 4 Recovery Options).
lane: koenigin
reasoning_effort: low
---

# Multi-Agent Pitfalls — Trigger-Watchlist

**Purpose:** Load this BEFORE every `delegate_task` call. Each pitfall has a **trigger symptom** (when it fires) and a **one-line fix** (what to do). Complement to `multi-agent-orchestration` which explains the pattern; this skill catches the mistakes.

**Don't read the whole list — scan triggers, apply what's relevant.**
...
| 26 | Subagent: "X errors in errors.log" (no date context, may be historical) | Ask for date breakdown. `grep ERROR log | awk '{print $1}' | sort -u | tail -10` — if all old, ignore |
...
1. **Phase A (current session):** Bauen — create the skill, document it, link it from `multi-agent-orchestration` if appropriate. Do NOT also spawn a subagent to demonstrate it in the same session.
2. **Phase B (next session):** Nutzen — run the actual live test, with the cheatsheet in mind.
...
### Protocol
1. **Pre-Scan:** `find ~/.hermes/skills -name "SKILL.md" -not -path "*/.archive/*" -exec wc -c {} \; | sort -rn | head -10`
2. **Struktur scannen:** `grep -n '^#\{1,3\} ' SKILL.md | head -40`
3. **Existierende `references/`, `templates/`, `scripts/` checken**
4. **Trennlinie definieren:**
...
5. **Subagent parallel pro Skill** (siehe Multi-Batch-Pattern oben)
6. **Verifizieren:** Frontmatter identisch? Refs valide? Keine Inhalte verloren?
   - **Phantom-Ref-Detection (in-line, run BEFORE step 7):** `grep -oE 'references/[a-zA-Z0-9_-]+\.md' SKILL.md | sort -u | while read r; do [ ! -f "$r" ] && echo "PHANTOM: $r"; done` — catches broken See-Also links inherited from pre-slim state. Proven 2026-07-03 on `hermes-v7-sse-server`: 1 phantom (`event-source-headers.md`) detected, created as new ref rather than block-replaced.
   - **Frontmatter byte-identity:** `sed -n '1,10p' SKILL.md > /tmp/fm.txt && diff <(cat /tmp/fm.txt) <(expected-fm)` → must be identical.
   - **Content loss check:** `grep -c '^## ' SKILL.md` + `grep -c '^| ' SKILL.md` before/after — heading/table counts should hold steady (or grow if summary moves were correct).
7. **Broken Refs fixen:** Bullet-Block-Replace mit Notiz (NICHT Ghost-Stubs) — siehe `Broken-Ref-Cleanup Pattern` unten für Stub-vs-Block-Replace-Entscheidung.
...
- 5 new `references/` files created: `phase-1-spawn-experts.md`, `phase-3-synthesis.md`, `queen-bee-configuration.md`, `pitfalls-detailed.md`, `report-template.md`
- Frontmatter byte-for-byte identical (verified via `diff`)
- 0 broken refs (verified via `ls references/<each>.md`)
...
- Subagent #1 (hermes-mcp-integration) demonstrated this on 2026-07-03 — produced MD5 `282410bc...` that matched original exactly
- **Use this as the new default** for slim-down verification

### Protocol
```bash
# 1. Extract frontmatter from original (if still available)
...
- Especially when the frontmatter has `metadata.hermes` blocks with complex nested structures
...
1. **Trailing whitespace** — `sed -i 's/[[:space:]]*$//' SKILL.md` and re-check
2. **BOM** — `file SKILL.md` should say "UTF-8" not "UTF-8 (with BOM)"
3. **Line endings** — `file SKILL.md` should say "Unix" not "DOS"
4. **YAML key order changed** — sed-reorder to original (auto-fix possible)
5. **Nested quote changes** — manual fix required
...
### Why this matters
- Without audit: Subagent may create duplicates of content that already lives in `references/`
- With audit: Subagent extracts only what's NEW, references existing files instead
- Subagent #3 (hermes-orchestration) demonstrated this on 2026-07-03 — found 56 run-artifacts in `memory/runs/` that weren't real references, focused extraction on the 7 actual references

### Protocol (Subagent must do this first)
```bash
# 1. List existing reference files (ignore run-artifacts / metadata / README)
SKILL_DIR=~/.hermes/skills/<category>/<skill>
ls "$SKILL_DIR/references/"  # all files

# 2. Categorize: real references vs run-artifacts vs meta-files
# Real: deep-dive docs (e.g., "phase-1-spawn-experts.md")
# Run-artifacts: dated session logs in memory/runs/ or similar
# Meta: README.md, INDEX.md, CHANGELOG.md

# 3. For each existing real-reference, scan its TOC / headings
grep -E '^#' "$SKILL_DIR/references/"*.md | head -50

# 4. Compare with SKILL.md content — what's ALREADY covered vs what needs NEW extraction
```

### Decision Tree
- Topic X is already fully covered in `references/topic-x.md` → **DON'T create new file**, link existing one
- Topic X is partially covered → **Extract only the missing parts**, mention the existing file in your report
- Topic X is not covered at all → **Create new `references/topic-x.md`**
...
- Subagent creates `references/<topic>.md` without mentioning audit
...
- ⚠️ Ref ist im Workflow verkettet (`[details](...)` im Fließtext, nicht in Bullet-Liste)
...
```bash
# Existiert der Skill überhaupt aktiv?
ls ~/.hermes/skills/<category>/<skill>/SKILL.md
# Falls nur in .archive/ oder duplicates-*: SKIP, kein Cleanup nötig
```
...
### Cross-Reference
- Full integration: `skill-library-maintenance` skill (v1.1.0), Pitfalls #17-#23
- R1 patterns: see "New Patterns Added 2026-07-02 (from live demo session)" section above
- **R2 patterns detailed walk-throughs:** see `references/2026-07-02-additions.md` §5a-5f for the deep-dive on each Round-2 pattern (Subagent-Strategie-Selbstwahl, Markdown-Compression-Iterations, Wave-Clock-Variance, Stub-vs-Block-Replace, Pre-Existing-Reference-Audit, MD5-Frontmatter-Verifikation)
- **Mission master reports:** `~/docs/system/skill-slim-down-2026-07-02.md` (R1, 9 skills, 513→95KB) + `~/docs/system/skill-slim-down-round-2-2026-07-03.md` (R2, 10 skills, 259→110KB). Cumulative across both rounds: 19 skills, 567 KB context-einsparung.
