...
description: |
  Use when locating the right Hermes skill for an unfamiliar request, comparing neighboring skill clusters, or finding a specialized workflow before acting.
  NOT for executing the chosen task, auditing skill quality, or treating this index as authoritative when the live skills list has changed.
  Provides a domain-grouped quick-reference map from common user intents to active Hermes skills.
version: 1.0.0
changelog:
...
metadata:
  hermes:
    source: minimax-hub
    hub_skill_id: orchestration-skill-navigator
    category: navigator
    domain: orchestration
    converted_at: '2026-07-03T23:19:32.984552'
  tags:
  - hub
  - conversion
  - workflow
...
- orchestration/skill
- navigator
trigger_keywords: ['skill', 'hermes', 'skills', 'locating', 'right']
keywords: ['skill', 'hermes', 'skills', 'locating', 'right']
...
> **Hub Origin:** Convexed from MiniMax Hub skill `orchestration/skill-navigator` (version 1.0.0). Original Hub-SKILL.md is preserved at `scripts-originals/SKILL.md.hub`, original meta.yaml at `scripts-originals/meta.yaml.hub`. All Hub-specific paths (e.g. `~/.hub-global/skills/orchestration-skill-navigator/`) translated to Hermes-equivalent references in `references/`.
...
### 🐝 Cluster 1: Multi-Agent & Orchestration (12 skills)
...
HERMES-V7 INTERNAL PATTERNS — Macro/Micro-Loop + Context-Health
├── hermes-long-run-template   ← 7-Phasen Long-Run (10–30 min Tasks)
├── hermes-react-pattern       ← ReAct Micro-Loop (Thought→Action→Obs)
├── hermes-context-budget      ← 85%-Compaction + Context-Health-Metriken
└── hermes-agentic-patterns    ← Brücke: Outcome-Prompts, NOTES, Spec→Fresh-Session
```
...
- **10–30 min Macro-Task mit mehreren Phasen?** → `hermes-long-run-template`
- **Lange Tool-Kette (>5 Calls)?** → `hermes-react-pattern` für ReAct-Labels
- **Context wird voll (>85%)?** → `hermes-context-budget` für Compaction
- **Welches agentic Pattern passt?** → `hermes-agentic-patterns` als Brücke
